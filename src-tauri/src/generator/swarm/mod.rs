...
pub trait Features {
    async fn new() -> (Pin<Box<Swarm<CentichainBehaviour>>>, PeerId); //config new swarm
    async fn dial<'a>(
        swarm: &mut Swarm<CentichainBehaviour>,
        db: &Database,
        window: &tauri::Window,
    ) -> Result<(), &'a str>; //dialing with a stable relay to comunicate with the network
}

impl Features for CentichainBehaviour {
    async fn new() -> (Pin<Box<Swarm<CentichainBehaviour>>>, PeerId) {
        ...
        let topic = IdentTopic::new("validator");
    ...
    async fn dial<'a>(
        ...
        match Addresses::contact(swarm, db, window).await {
            Ok(_) => Ok(()),
            Err(e) => Err(e),
        }
    }
